...
//    NSString *time = [formatter stringFromDate:self.lastUpdateTime];
    NSString *time1=[self intervalSinceNow:self.lastUpdateTime];
    // 3.显示日期
    if (_timeLabelText.length>0) {
        self.lastUpdateTimeLabel.text = _timeLabelText;
    }else{
        self.lastUpdateTimeLabel.text = [NSString stringWithFormat:@"最后更新：%@", time1];
        _timeLabelText=self.lastUpdateTimeLabel.text;
    }
}
